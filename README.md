...
$T_w = T_c \times \mathrm{gearRatio} \times finalDrive$. Wheels
...
The orientation and position after a preview time $t_p$ are estimated using
$$
\hat{\theta} = \theta + \tfrac{v}{L}\tan(\delta)\, t_p
$$
$$
\hat{p} = p + v t_p [\cos \hat{\theta},\, \sin \hat{\theta}]
$$
The controller searches the path ahead for a target point $(x_l, y_l)$ and computes
$$
\alpha = \operatorname{atan2}(y_l - \hat{p}_y,\, x_l - \hat{p}_x) - \hat{\theta}
$$
which yields the steering command
$$
...
```
##### Heading prediction
Projects the orientation forward using
$\hat{\theta} = \theta + \tfrac{v}{L}\tan(\delta) t_p$.

##### Position prediction
Estimates the future position from the predicted heading:
$\hat{p} = p + v t_p[\cos\hat{\theta},\, \sin\hat{\theta}]$.

##### Lookahead search
Searches the reference path for a point $(x_l, y_l)$ at distance $d_l$ and computes
$\alpha = \operatorname{atan2}(y_l - \hat{p}_y,\, x_l - \hat{p}_x) - \hat{\theta}$.

##### Pure Pursuit steering
Applies the steering law
$\delta_{pp} = \operatorname{atan2}(2L \sin \alpha,\, d_l)$.

##### planPathWithPredictions
Adjusts the reference path using previewed points before filtering.

##### Zig-zag correction
Suppresses short oscillations in the planned curvature.

##### Gaussian & low-pass
Smooths the steering angle.

##### Gear shift rules
Refines gear selection using the previewed curvature.

##### Steering command $\delta_{cmd}$
Final filtered command sent to the Ackermann steering.
